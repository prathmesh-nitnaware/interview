...
from backend.ollama_client import (
    evaluate_answer, 
    generate_question_list, # <-- UPDATED
    get_coding_problem,
...
from ml.nlp.resume_parser import parse_resume
...
# (We are now "stateless". The frontend will hold the data until the end.)
...
def _generate_final_report(conversation_history: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    avg_nervousness = np.mean([turn["scores"]["audio_nervousness"] for turn in conversation_history])
    ...
    # 2. "Nervousness Model"
    blink_nervousness_score = np.clip((avg_blinks - 20) / 30.0, 0.0, 1.0)
    overall_nervousness_score = (0.6 * avg_audio_nervousness) + (0.4 * blink_nervousness_score)
    ...
    return {
        ...
        "overall_nervousness_score": overall_nervousness_score,
...
# --- ENDPOINT 1: GENERATE THE *ENTIRE* INTERVIEW ---
@app.post("/generate_interview_questions")
async def generate_interview_questions_endpoint(
    difficulty: str = Form(...),    # "easy", "medium", or "hard"
    num_questions: int = Form(...), # e.g., 3
    ...
    try:
        ...
        # 2. Generate List of Question Strings
        ...
        # 3. Generate Audio for *each* question
        interview_plan = []
        ...
        # 4. Return the full plan to the frontend
        return {
            "resume_data": parsed_data,
            "interview_plan": interview_plan # This is a list of {text, audio}
        }
...
# --- ENDPOINT 2: SUBMIT THE *ENTIRE* INTERVIEW FOR GRADING ---
@app.post("/submit_full_interview")
async def submit_full_interview_endpoint(
    # The frontend must now send a JSON string that maps to the files
    turn_data_json: str = Form(...), # e.g., '[{"q": "Q1", "cv_key": "cv_1"}, {"q": "Q2", "cv_key": "cv_2"}]'
    # And then send all the files
    files: List[UploadFile] = File(...)
):
    """
    This is the new "brain". It receives all data at the end,
    ...
    try:
        ...
        # Map filenames to their file objects for easy lookup
        file_map = {f.filename: f for f in files}
        
        conversation_history = []
        
        # Loop through each "turn" sent from the frontend
        for turn in turn_data_list:
            ...
            # --- 5. Get Answer Content Score ---
            content_score = evaluate_answer(question_text, user_answer_text)
            
            os.remove(audio_tmp_path) # Clean up temp audio file

            # --- 6. Save this turn's data ---
        ...
        # --- 7. All turns processed, generate the final report ---
...
# --- Utility Endpoint (for frontend CV) ---
@app.post("/analyze_frame")
async def analyze_frame_endpoint(file: UploadFile = File(...)):
    """Analyzes a single video frame for CV scores. This is a simple utility."""
